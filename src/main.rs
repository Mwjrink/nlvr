...
impl VulkanApp {
    ...
    fn create_swapchain_and_images(
        ...
        queue_families_indices: QueueFamiliesIndices,
        dimensions: [u32; 2],
    ) -> (Swapchain, vk::SwapchainKHR, SwapchainProperties, Vec<vk::Image,>,)
    {
    ...
    fn create_texture_image(vk_context: &VkContext, command_pool: vk::CommandPool, copy_queue: vk::Queue,) -> Texture {
        let cursor = fs::load("images/ucykfbkfa_ZTool.BMP",);
        let image = image::load(cursor, image::ImageFormat::Bmp,).unwrap().flipv();
    ...
    ) -> (vk::Image, vk::DeviceMemory,)
    {
        ...
        let mem_type_index = Self::find_memory_type(mem_requirements, vk_context.get_mem_properties(), mem_properties,);
    ...
    )
    {
        ...
        Self::execute_one_time_commands(vk_context.device(), command_pool, transfer_queue, |buffer| {
    ...
    fn load_model() -> (Vec<Vertex,>, Vec<u32,>,) {
        ...
        let mut cursor = fs::load("models/ucykfbkfa_ZTool.OBJ",);
        let (models, _,) =
    ...
    fn create_uniform_buffers(vk_context: &VkContext, count: usize,) -> (Vec<vk::Buffer,>, Vec<vk::DeviceMemory,>,) {
    ...
    pub fn draw_frame(&mut self,) -> bool {
...

        {
            let present_info = vk::PresentInfoKHR::builder()
                ...
                .build();
            let result = unsafe { self.swapchain.queue_present(self.present_queue, &present_info,) };
            match result {
    ...
    fn update_uniform_buffers(&mut self, current_image: u32,) {
        ...
        let aspect = self.swapchain_properties.extent.width as f32 / self.swapchain_properties.extent.height as f32;
        let ubo = CameraUBO {
